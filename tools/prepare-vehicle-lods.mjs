...
for(const key of ['porsche','bmw','gt40','f40','r34','supra-mk4','r8','mcf1'].filter(k=>!process.argv[2]||process.argv.slice(2).includes(k))){
 ...
 await doc.transform(prune());const before=count();await doc.transform(simplify({simplifier:MeshoptSimplifier,ratio:.28,error:.003}),prune(),draco({quantizePosition:16,quantizeNormal:12}));
