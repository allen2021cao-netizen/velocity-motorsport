...
export function mountExperience(setQuality:(value:string)=>void){
 ...
 panel.innerHTML='<button id="fpsToggle" aria-pressed="false">显示帧数</button><output id="fpsValue" hidden></output><div id="fpsAdvice" role="status" hidden><span></span><button>切换画质</button><button aria-label="关闭帧率提示">×</button></div>';
 ...
 let show=false,seconds=0,frames=0,lowSeconds=0,cooldown=0,lastQuality='',target='balanced';
