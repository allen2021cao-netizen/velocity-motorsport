...
interface Voice {osc:OscillatorNode;sub:OscillatorNode;filter:BiquadFilterNode;gain:GainNode;pan:StereoPannerNode;}
...
export class RaceAudio {
 ...
 constructor(private ac:AudioContext,master:AudioNode){
  ...
  for(const name of ['muscle','exotic','six','tire','road','impact','shift'])void this.load(name);
 ...
 private async load(name:string){try{const r=await fetch('/audio/'+name+'.wav');if(!r.ok)throw Error(String(r.status));const buffer=await this.ac.decodeAudioData(await r.arrayBuffer());this.buffers.set(name,buffer);if(['muscle','exotic','six','tire','road'].includes(name)){const source=this.ac.createBufferSource(),gain=this.ac.createGain();source.buffer=buffer;source.loop=true;gain.gain.value=0;source.connect(gain);if(name==='tire'||name==='road'){const filter=this.ac.createBiquadFilter();filter.type='lowpass';filter.frequency.value=name==='tire'?4200:1800;gain.connect(filter);filter.connect(this.effects);}else gain.connect(this.engineBus);source.start();this.samples.set(name,{source,gain});}this.loaded.push(name);}catch{this.failed.push(name);}}
 ...
 update(frame:Frame,dt:number){const newRace=frame.countdown&&!this.wasCountdown;this.wasCountdown=frame.countdown;this.current=frame;if(!frame.active||document.hidden){this.silence();return;}const a=this.ac,p=ENGINE_PROFILES[frame.type]||ENGINE_PROFILES['458'];
  ...
  const shiftDip=this.train.shift>0?.45:1,available=this.samples.has(p.family);this.smooth(this.engineBus.gain,alive*(frame.camera===1?.82:1)*shiftDip,.025);
  this.tune(this.synth,p,this.train.rpm,(available?.025:.15)*(.4+load*.6),0);
  for(const [name,v] of this.samples){if(name==='tire'||name==='road')continue;this.smooth(v.source.playbackRate,(.58+n*1.24)*p.pitch,.055);this.smooth(v.gain.gain,name===p.family?(.10+load*.19+n*.035)*p.body:0,.10);}
  this.smooth(this.synth.filter.frequency,350+n*3000+load*1600);const slip=clamp((frame.slip-.7)/8,0,1)*clamp(speed/6,0,1);
 ...
 private tune(v:Voice,p:EngineProfile,rpm:number,gain:number,pan:number){const f=rpm/60*p.cylinders/2;this.smooth(v.osc.frequency,f,.06);this.smooth(v.sub.frequency,f*.5,.06);this.smooth(v.filter.frequency,800+rpm*.32);this.smooth(v.gain.gain,gain,.08);this.smooth(v.pan.pan,pan,.1);}
