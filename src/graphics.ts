...
export function setupGraphics(renderer:T.WebGLRenderer,scene:T.Scene,camera:T.Camera,sun:T.DirectionalLight){
 ...
 let quality='high';
 return {quality(q:string){quality=q;renderer.shadowMap.enabled=q!=='low';},resize(){composer.setPixelRatio(renderer.getPixelRatio());composer.setSize(innerWidth,innerHeight);},render(focus:T.Vector3,overview=false){renderer.info.reset();const extent=overview?460:32;sun.shadow.bias=overview?-.0007:-.0001;sun.shadow.normalBias=overview?.5:.035;sun.shadow.camera.left=-extent;sun.shadow.camera.right=extent;sun.shadow.camera.top=extent;sun.shadow.camera.bottom=-extent;sun.shadow.camera.far=overview?1600:200;sun.shadow.camera.updateProjectionMatrix();sun.position.copy(focus).add((sun.userData.offset??new T.Vector3(-35,60,25)).clone().multiplyScalar(overview?10:1));sun.target.position.copy(focus);if(quality==='high')composer.render();else renderer.render(scene,camera);}};
}
