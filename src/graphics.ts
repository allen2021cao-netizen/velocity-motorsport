...
export function setupGraphics(renderer:T.WebGLRenderer,scene:T.Scene,camera:T.Camera,sun:T.DirectionalLight){
 ...
 let quality='high';
 return {quality(q:string){quality=q;renderer.shadowMap.enabled=q!=='low';},resize(){composer.setPixelRatio(renderer.getPixelRatio());composer.setSize(innerWidth,innerHeight);},render(focus:T.Vector3,overview=false,studio=false,viewport?:DOMRect){const original={background:scene.background,environment:scene.environment,fog:scene.fog,intensity:scene.environmentIntensity,rotation:scene.environmentRotation.y,sun:sun.intensity,color:sun.color.clone()};if(studio){scene.background=new T.Color(0x1c232a);scene.environment=studioEnvironment;scene.environmentIntensity=1.25;scene.environmentRotation.y=0;scene.fog=null;sun.intensity=2.4;sun.color.set(0xfff5e9);}renderer.info.reset();const extent=overview?460:32;sun.shadow.bias=overview?-.0007:-.0001;sun.shadow.normalBias=overview?.5:.035;sun.shadow.camera.left=-extent;sun.shadow.camera.right=extent;sun.shadow.camera.top=extent;sun.shadow.camera.bottom=-extent;sun.shadow.camera.far=overview?1600:200;sun.shadow.camera.updateProjectionMatrix();sun.position.copy(focus).add((sun.userData.offset??new T.Vector3(-35,60,25)).clone().multiplyScalar(overview?10:1));sun.target.position.copy(focus);if(viewport){renderer.setScissorTest(false);renderer.setViewport(0,0,innerWidth,innerHeight);renderer.setClearColor(0x0d161d,1);renderer.clear();const y=innerHeight-viewport.bottom;renderer.setViewport(viewport.left,y,viewport.width,viewport.height);renderer.setScissor(viewport.left,y,viewport.width,viewport.height);renderer.setScissorTest(true);renderer.render(scene,camera);renderer.setScissorTest(false);renderer.setViewport(0,0,innerWidth,innerHeight);}else if(quality==='high')composer.render();else renderer.render(scene,camera);if(studio){scene.background=original.background;scene.environment=original.environment;scene.environmentIntensity=original.intensity;scene.environmentRotation.y=original.rotation;scene.fog=original.fog;sun.intensity=original.sun;sun.color.copy(original.color);}}};
}
