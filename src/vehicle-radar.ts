...
export function updatePerformanceRadar(car:VehiclePerformance){
 ...
 if(!root){
  ...
  root.innerHTML=`<div class="radar-caption">车辆性能 <span>游戏内评分 / 100</span></div><svg viewBox="0 0 320 264" role="img" aria-labelledby="radarTitle"><title id="radarTitle"></title>${[.25,.5,.75,1].map(s=>`<polygon class="radar-grid" points="${polygon(82*s)}"/>`).join('')}${PERFORMANCE_AXES.map((_,i)=>`<line class="radar-grid" x1="160" y1="133" x2="${point(i,82)[0]}" y2="${point(i,82)[1]}"/>`).join('')}<polygon id="radarShape"/>${PERFORMANCE_AXES.map((a,i)=>{const [x,y]=point(i,111);return `<text x="${x}" y="${y-3}" text-anchor="middle">${a.name}<tspan class="radar-score" id="radarScore${i}" x="${x}" dy="17"></tspan></text>`;}).join('')}</svg><details><summary>六项如何影响驾驶</summary><ul>${PERFORMANCE_AXES.map(a=>`<li><b>${a.name}</b> · ${a.description}</li>`).join('')}</ul><p>统一标尺；分数越高，该项越强。车辆调校评分，非厂家实测。</p></details>`;
 ...
 scores.forEach((v,i)=>root!.querySelector('#radarScore'+i)!.textContent=String(v));
