...
test('classic GLBs have grounded articulated wheels, valid geometry and smaller distance bodies',async()=>{
 ...
 for(const [key,length] of [['f40',4.43],['r34',4.6],['supra-mk4',4.52],['r8',4.43],['mcf1',4.365],['veneno',5.02],['slr',4.656],['challenger',5.029],['c7',4.493]] as const){
  ...
  assert.ok(smallBody<fullBody*(key==='slr'?.9:.65),`${key}: ${smallBody}/${fullBody}`);
 }
});
