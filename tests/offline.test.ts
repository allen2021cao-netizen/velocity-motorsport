...
test('offline worker validates assets, resumes partial downloads and serves a navigation without network',async()=>{
 ...
 const live=await event('fetch',{request:{url:'https://game.test/',method:'GET',mode:'navigate'}});assert.match(await live.text(),/challenge-platform/, 'online response preserves Cloudflare security script');
 online=false;const model=await event('fetch',{request:{url:'https://game.test/models/test.glb',method:'GET',mode:'cors'}});assert.equal(await model.text(),'model bytes');assert.equal(requests,before+1);
