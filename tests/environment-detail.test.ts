...
test('detail changes use a distance margin and preview restores full geometry',()=>{
 ...
 detail.update(new Vector3(300,0,0));assert.equal(mesh.geometry,levels[1]);
 detail.update(new Vector3(240,0,0));assert.equal(mesh.geometry,levels[1]);
 detail.update(new Vector3(190,0,0));assert.equal(mesh.geometry,levels[0]);
