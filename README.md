...
- 13 vehicles: four imported GLBs (Ferrari 458, Porsche 911 Carrera 4S, BMW M4 Competition and Ford GT40 Mark II), and nine selected procedural models. The additional three GLBs total 5.65 MB and replace the former 993, M3 and GT40 Mk I labels. Source credits and licenses: `public/models/CREDITS.md`. Rebuild assets with `node tools/fetch-vehicles.mjs` followed by `node tools/prepare-vehicles.mjs`. The underlying procedural collection contains 12 individually parameterized coachwork models with continuous body surfaces, open wheel arches, separate roof/glazing, seats/instruments/steering, model-specific lighting/aero and independently rotating wheels with stationary calipers; plus a Draco-compressed Ferrari 458 GLB. `src/automotive/` replaces the previous side-profile extrusion at runtime. The twelve generated vehicles are authored approximations, not manufacturer CAD or scanned replicas.
